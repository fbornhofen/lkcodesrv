module.exports = function(db, logger) {
  ...
  var setup = CodeDatabase.setup = function() {
  ...
  var teardown = CodeDatabase.teardown = function() {
    db.query("DROP TABLE files", null, function(err) {if (err) {
      logger.error(err);
    }});
  }
  ...
  var getLatestRevisionNumber = CodeDatabase.getLatestRevisionNumber = function(next) {
    logger.trace('getLatestRevisionNumber entry');
    db.fetchAll("SELECT MAX(revision) FROM files", null, function(err, dbres) {
      logger.trace('getLatestRevisionNumber callback');
      if (err) {logger.error(err);}
      if (dbres && dbres[0]) {
        next(dbres[0]["MAX(revision)"]);
      } else {
        logger.error('getLatestRevisionNumber: no revisions present');
        next(-1);
      }
    });
  }
  ...
  var listFilesInPath = CodeDatabase.listFilesInPath = function(path, revNr, next) {
    ...
    db.fetchAll("SELECT path FROM files WHERE path LIKE '" + path + "%' AND revision <= " + revNr, null, function(err, dbres) {
      var children = {};
      if (err) {
        logger.error(err);
        next([]);
