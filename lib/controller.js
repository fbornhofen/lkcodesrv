...
module.exports = function(codeDatabase, logger) {
  ...
  var evalJs = Controller.evalJs = function(aString, response, onError) {
    var db = codedb.getDatabase();
    try {
      var func = eval('(' + aString + ')');
      func(response);
    } catch (e) {
      logger.error('Error evaluating "' + aString + '"'); 
      return onError('Error evaluating "' + aString + '": ' + JSON.stringify(e));
    }
  }
